...
size_src_old := -3
size_dst_old := -4
buffersize := 150000000					; 20MB for HD
...
if ((size_dst < size_src) or (size_src != size_src_old))		; (src fils is growing) or (dst file size < src file size)
{
	...
	SetTimer, check_src, -200					; large value for small bitrate media (file size incresing can not be detect during short period)  (200 for HD)
}
else
{
