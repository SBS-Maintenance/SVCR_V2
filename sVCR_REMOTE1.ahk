...
remotenumber := SubStr(A_ScriptName, 12, 1)    ; get remote number from script name
OutputDebug, %remotenumber%

global iSendSocket = 0, ipaddress, port

IniRead, ipaddress, remote%remotenumber%.ini, network, address, localhost
IniRead, port, remote%remotenumber%.ini, network, port, 3441
IniRead, remotename, remote%remotenumber%.ini, network, name, REMOTE%remotenumber%
...
remote_panel_x := 250 + (remotenumber - 1) * 200
...
gui, font, s20
gui, add, text, xm ym+150 hwndhtext,  %remotename%
gui, show, x%remote_panel_x% y%remote_panel_y%  w160 h200
gui, -MinimizeBox
...
if (sendresult := sendcommand(ipaddress, "_SETT_" . paneltext))
{
...
sendcommand(ipaddress, command)
{
    ...
    {
        MsgBox Error Opening Socket with code %i%, %ipaddress%
    }
 ...
 if (sendresult <= 0) or (iSendSocket <= 0)               ; fail to open socket or fail to send command
    MsgBox,,CAUTION !!, Device Not Ready
