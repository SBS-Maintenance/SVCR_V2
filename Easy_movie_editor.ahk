...
;   2020/7/6   prepare for compiled version
...
binary_showjob = %A_WorkingDir%\show_job.%ahk_exe%


title_preview := "Editor Play >>>>"
...
wtitle = Easy Movie Editor v2 (20200706)
...
mpv_filter["stereo-2"] := mpv_filter["mono-2"]
...
encoder_filter["4 channels-2"] := encoder_filter["2 channels-2"]
...
gui, add, DropDownList, xm+160 yp-4 Choose1 vout_type hwndhout_type AltSubmit, STREAM COPY|PROXY MP4|STREAM COPY_A1
...
if !media.fullpath
	return
Process, Exist, %play_pid%			; check if there is mpv pid
if errorlevel > 0
	return
...
runstring = %binary_mpv% %filter%  --keep-open --pause --hr-seek=yes --osd-level=3 --osd-fractions --start %starttime% --title "%title_preview% %file_name%"
if (media.width > 1300)
	runstring = %runstring% --window-scale=0.5
if (media.scantype = "interlaced")
	runstring .= " --deinterlace=yes "
runstring = %runstring% --script="%A_WorkingDir%\mark_inout.lua" "%file_full%"
...
{
	GuiControl,, %hstatus%, % "Error Opening Preview window, PID " . play_pid
	play_pid := -1
}
else
	GuiControl,, %hstatus%, % "Preparing for Preview window, PID " .  play_pid
...
if WinExist(title_preview)
{
	WinSet, Style, -0x20000, %title_preview%					; remove minimize button
	WinSet, Style, -0x30000, %title_preview%				; remove maximize button
	GuiControl,, %hstatus%, % "Preview window Opened "  . (A_TickCount -tick_start) / 1000 . " sec"
}
else
{
	GuiControl,, %hstatus%, % "Preparing for Preview window  --- " . (A_TickCount - tick_start) / 1000 . " sec"
	SetTimer, mpvchk_title, -50
}

if (((A_TickCount - tick_start) / 1000) > 60)					; no more waiting preview window (30 second)
{
	SetTimer, mpvchk_title, off
...
if (out_type = 2)				; output type is not stream copy
	file_extension = mp4
...
if outfilenamechange
{
	outfilename := Trim(outfilename)                               ; trim left or right space, tab
...
if (out_type = 3)		; STREAM COPY MODE WITH SINGLE AUDIO STREAM
	ffmpeg_PID := transcoding_normal_single(starttime, duration, file_full, targetfile)
...
Run, "%binary_showjob%" "%ffmpeg_pid%" "Editing is progress...." "%file_name_only%"
ToolTip, Editing started with pid %ffmpeg_PID%`, %file_name_only%_cut_%cut_prefix%.%file_extension%
SetTimer, removetooltip, -1000
SetTimer, video_preview, -200
return


video_preview:
WinActivate, %title_preview%
return
...
transcoding_normal_single(starttime, duration, file_i, file_o)
{
	global binary_ffmpeg
	runstring = %binary_ffmpeg% -ss %starttime% -i "%file_i%" -t %duration% -map 0:v -map 0:a:0 -c:v copy -c:a copy -ignore_unknown -y "%file_o%"
	FileAppend, %runstring%, *
	run, %runstring%,,Minimize,ffmpeg_PID
	return ffmpeg_PID
}

transcoding_proxy(starttime, duration, file_i, file_o)		; New 2019/6/2
{
	global binary_ffmpeg, media, encoder_filter, media, audio_monitor, audiomonl, audiomonr
	
	if (StrLen(encoder_filter[media.audio_format]) > 3)			; apply pan filter if there is "amerge" or "anull" filter
		audio_monitor_filter := ";[as2]pan=stereo|c0=" . audio_monitor[audiomonl] . "|c1=" . audio_monitor[audiomonr] .  """"
	else
		audio_monitor_filter := ""
	runstring = %binary_ffmpeg% -ss %starttime% -i "%file_i%" -t %duration%
	runstring .=  encoder_filter[media.scantype] encoder_filter[media.audio_format] audio_monitor_filter
	runstring = %runstring%   -pix_fmt yuv420p -c:v libx264 -preset:v veryfast -crf 24 -c:a aac -ac 2 -y "%file_o%"
	run, %runstring%,,Minimize,ffmpeg_PID
	return ffmpeg_PID
return
...
SplitPath, file_full, file_name, file_path, file_extension, file_name_only
GuiControl,, %hstatus%, Loading File... %file_name%
ToolTip					; added 2019/3/30
Sleep, 200				; added 2019/10/29			apply some delay for button disabling animation
...
outfolder := selectfolder(outfolder)
